//
//  FindDrawLineCompositeController.m
//  MapKitTest
//
...
@interface FindDrawLineCompositeController ()<MAMapViewDelegate,AMapSearchDelegate,AMapLocationManagerDelegate,AMapNaviDriveManagerDelegate,DriveNaviViewControllerDelegate>
...
@implementation FindDrawLineCompositeController
...
- (void)initCompleteBlock {
    __weak FindDrawLineCompositeController *weakSelf = self;
...
- (void)_initBottonView {
    ...
    self.describeLabel.font = [UIFont systemFontOfSize:18.0];
    ...
    [self.mapView.overlays enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id<MAOverlay> overlay, NSUInteger idx, BOOL *stop) {
        ...
        if ([overlay isKindOfClass:[MultiDriveRoutePolyline class]]) {
            ...
            if ([multiPolyline.routeIDNumber integerValue] == routeID) {
                ...
                //更新CollectonView的信息
                self.describeLabel.text = [NSString stringWithFormat:@"长度:%ld米 | 预估时间:%ld分钟 | 分段数:%ld", (long)aRoute.routeLength, (long)aRoute.routeTime/60, (long)aRoute.routeSegments.count];
